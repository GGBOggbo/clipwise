...
| 项目 | 命令 | 结果 |
|---|---|---|
| Worker 全量测试 | `cd services/worker && env -u ALL_PROXY -u all_proxy -u HTTP_PROXY -u http_proxy -u HTTPS_PROXY -u https_proxy -u NO_PROXY -u no_proxy uv run pytest -q` | 待运行 |
| Web 单测 | `DATABASE_URL='postgres://clipwise:clipwise_dev@localhost:5432/clipwise' pnpm --filter @clipwise/web exec vitest run --exclude 'tests/integration/**'` | 待运行 |
| Web 非真实集成 smoke | `DATABASE_URL='postgres://clipwise:clipwise_dev@localhost:5432/clipwise' pnpm --filter @clipwise/web exec vitest run tests/integration/create-to-ready.test.ts tests/integration/sse-flow.test.ts` | 已于实现中通过，最终验收待重跑 |
| E2E | `DATABASE_URL='postgres://clipwise:clipwise_dev@localhost:5432/clipwise' pnpm test:e2e` | 待运行 |
| Lint | `pnpm lint` | 待运行 |
| Build | `pnpm build` | 待运行 |
| Diff whitespace | `git diff --check` | 待运行 |
| DB migration drift | `pnpm db:generate && git status --short apps/web/db/migrations` | 待运行 |

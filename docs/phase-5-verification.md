...
状态：✅ 已完成。项目 `TY2atDjXUvGv`：浏览器上传 → Groq ASR（326 条转写）→ DeepSeek 编辑师召回管线 → 成功。

| 验证点 | 结果 |
|---|---|
| 候选数量（≤30） | ✅ 8 条（对 8 分钟视频自然，未硬凑到 30） |
| 每条带 recommendation | ✅ strong/recommended 真实分布 |
| 每条带 topicLabel | ✅ 如"AI开发0到40k核心方法论"、"40k底气与领先溢价" |
| 每条带 editingNote | ✅ 如"视频中多处口误和错别字（'星期'应为'兴起'…），建议发布前校正" |
| 每条带 boundaryReason | ✅ 如"从开头强钩子'从0到40k'到…结束，完整" |
| 每条带 needsSetup | ✅ rank 3 needsSetup=true（内容与前文重叠，需补开场） |
| highlight_window_scores 行数 | ✅ 10（= 被评分窗口数） |
| 选择状态分布 | ✅ selected=8 / rejected=1 / scored=1（含留痕） |
| 项目最终状态 | ✅ ready |

过程中处理的一次"处理失败"：worker 进程启动于 16:47，晚于 Phase 5.1 代码提交（18:xx），跑的是旧管线；且 DeepSeek 在 details 阶段某次返回了非 transcript 原文金句，被 `quote_is_verbatim` 校验拦截（这正是"失败绝不产假数据"的安全行为）。重启 worker 加载 Phase 5.1 代码 + 重置任务后，模型返回合规内容，一次成功。

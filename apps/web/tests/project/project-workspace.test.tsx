...
describe("ProjectWorkspace", () => {
  ...
  it("显示五阶段进度和候选区域", () => {
    ...
    expect(
      screen.getByText(mockReadyProject.sourceFileName),
    ).toBeVisible();
    expect(screen.queryByText("演示项目")).not.toBeInTheDocument();
    expect(screen.getByText("选择回放")).toBeVisible();
    expect(screen.getByText("分析内容")).toBeVisible();
    expect(screen.getByText("生成候选")).toBeVisible();
    expect(screen.getByText("预览确认")).toHaveAttribute(
      "aria-current",
      "step",
    );
  ...
  it("ready 项目默认进入第一个候选的审片状态", () => {
    ...
    expect(screen.getByLabelText("标题 1")).toHaveValue(
      "为什么很多人做 AI 应用第一步就错了",
    );
